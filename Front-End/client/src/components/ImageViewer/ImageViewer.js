...
const Canvas = () => {
  ...
  const [mouseDowmPos, setMouseDowmPos] = useState[(0, 0)];
  /**Record the distance before this translation*/
  const [offsetDis, setOffsetDis] = useState[(0, 0)];
  /**Picture node*/
  ...
  const handleMouseMove = (event) => {
    ...
    const labelLeft =
      parseInt(`${imgInfo.lableLeft}`, 10) * imgScale * totalZoomRateX +
      offsetX;
    const labelTop =
      parseInt(`${imgInfo.lableTop}`, 10) * imgScale * totalZoomRateY + offsetY;
    ctx.strokeStyle = "#da2727";
    ctx.strokeRect(
      labelLeft,
      labelTop,
  ...
  const handleWheelImage = (event) => {
    ...
    const labelLeft =
      ...
      offsetDis.left;
    const labelTop =
      parseInt(`${imgInfo.lableTop}`, 10) * imgScale * totalZoomRateY +
      offsetDis.top;
    ctx.strokeStyle = "#da2727";
    ctx.strokeRect(
      labelLeft,
